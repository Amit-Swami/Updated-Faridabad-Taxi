...
import com.example.faridabadtaxirider.Remote.RetrofitClient;
...
public class RequestDriverActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    @BindView(R.id.btn_confirm_taxi)
    Button btn_confirm_taxi;
    ...
    private void addPulsatingEffect(SelectedPlaceEvent selectPlaceEvent) {
    ...
    private void startMapCameraSpinningAnimation(SelectedPlaceEvent selectPlaceEvent) {
    ...
    private void findNearbyDriver(SelectedPlaceEvent selectPlaceEvent) {
        if (Common.driversFound.size() > 0)
        {
            float min_distance = 0; //default min distance
            ...
            for (String key:Common.driversFound.keySet())
            {
                ...
                if (min_distance == 0)
                {
                    ...
                    if (!Common.driversFound.get(key).isDecline())//If not decline befor
                    {
                        foundDriver = Common.driversFound.get(key);
                        break;//exit loop becos we found driver
                    }
                    else
                ...
                {
                    //if have any driver with smaller min_distance, just get it
                    min_distance = driverLocation.distanceTo(currentRiderLocation);//first default min_distance
    ...
    private SelectedPlaceEvent selectPlaceEvent;
                ...
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists())
                        {
                            ...
                            Snackbar.make(main_layout,getString(R.string.trip_not_found)+event.getTripIp(),Snackbar.LENGTH_LONG).show();
                    }
        ...
        compositeDisposable.add(iGoogleAPI.getDirections("driving",
                ...
                .subscribe(returnResult -> {
                    ...
                    try {
                        ...
                        driverOldPosition = to; //set neww driver position
    ...
    @Subscribe(sticky = true,threadMode =  ThreadMode.MAIN)
    public void onSelectPlaceEvent(SelectedPlaceEvent event)
    {
    ...
    public void onDeclineRequestEvent(DeclineRequestFromDriver event)
    {
        if(lastDriverCall != null)
        {
            ...
            //if driver has been decline request, just find new driver
            findNearbyDriver(selectPlaceEvent);
        }
    }
    ...
    public void onDeclineRequestAndRemoveTripEvent(DeclineRequestAndRemoveTripFromDriver event)
    {
        ...
        {
            Common.driversFound.get(lastDriverCall.getKey()).setDecline(true);
            //if driver has been decline request, just finish this activity
            finish();
        }
    }
    ...
    private void drawPath(SelectedPlaceEvent selectPlaceEvent) {
        ...
        compositeDisposable.add(iGoogleAPI.getDirections("driving",
                ...
                .subscribe(returnResult -> {
                    ...
                    try {
                        ...
                        selectPlaceEvent.setDistanceText(distance);
                        selectPlaceEvent.setDurationText(duration);
                        ...
                        //update here
                        selectPlaceEvent.setOriginAddress(start_address);
                        selectPlaceEvent.setOrigin(new LatLng(startLocation.getDouble("lat"),
                                startLocation.getDouble("lng")));


                        selectPlaceEvent.setDestinationAddress(end_address);
                        selectPlaceEvent.setDestination(new LatLng(endLocation.getDouble("lat"),
                                endLocation.getDouble("lng")));

                        selectPlaceEvent.setDurationValue(durationValue);
                        selectPlaceEvent.setDistanceValue(distanceValue);

                        //calculate fee
                        double fee = Common.calculateFeeBaseOnMetters(distanceValue);
                        selectPlaceEvent.setTotalFee(fee);
                        txt_fee.setText(new StringBuilder("$").append(fee));
